...
const EightFirst = () => {
  ...
  const handleSubmit = () => {
    if (selectedUploads.length === 8) {
      // Handle the submit logic here (e.g., send selected uploads to the server)
      alert("Successfully submitted the 8 selected uploads!");
      // Perform any other submit actions here, like sending data to the server
    } else {
      alert("Please select 8 uploads before submitting.");
    }
  };
